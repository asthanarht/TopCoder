...
public class PaintingBoards {
   public double minimalTime(int[] boardLength, int[] painterSpeed) {
      ...
      for (int i = 0; i < n; i++)
         for (int j = 0; j < m; j++)
            for (int k = j + 1; k <= m; k++)
               time[count++] = (len[k] - len[j]) / (double) painterSpeed[i];
      ...
      while (left < right) {
         ...
         // where could the ith painter start from jth board could paint to in t
         // time
         for (int i = 0; i < n; i++)
            for (int j = 0; j <= m; j++) {
               paintTo[i][j] = j;
         ...
         for (int mask = 1; mask < 1 << n; mask++) {
            ...
            for (int i = 0; i < n; i++)
               if ((mask & (1 << i)) > 0)
                  dp[mask] = Math
                        .max(dp[mask], paintTo[i][dp[mask ^ (1 << i)]]);
            // dp[mask ^ (1 << i)] best could be done without the ith painter
         }
