...
class World {
	...
	public void applyMove(int moveId, int r1, int c1, int dir) throws Exception {
		if (r1 < 0 || r1 >= N || c1 < 0 || c1 >= N) {
			throw new Exception("In move " + moveId
					+ " (0-based), the cell chosen by your solution is outside"
		...
		if (r2 < 0 || r2 >= N || c2 < 0 || c2 >= N) {
			throw new Exception("In move " + moveId
					+ " (0-based), the direction chosen by your solution"
					+ " points to a cell outside the board: row = " + r2
					+ ", col = " + c2 + ".");
		}
...
class Drawer extends JFrame {
	...
	class DrawerPanel extends JPanel {
		final Color[] colors = new Color[] { new Color(255, 127, 127),
				new Color(127, 255, 127), new Color(127, 127, 255),
				new Color(255, 255, 127), new Color(127, 255, 255),
				new Color(255, 127, 255) };

		public void drawGrid(Graphics g, int topX, int topY) {
			g.setColor(Color.BLACK);
			for (int i = 0; i <= boardSize; i++) {
				g.drawLine(topX + i * cellSize, topY, topX + i * cellSize, topY
						+ cellSize * boardSize);
				g.drawLine(topX, topY + i * cellSize, topX + cellSize
						* boardSize, topY + i * cellSize);
			}

			for (int i = 0; i < boardSize; i++) {
				for (int j = 0; j < boardSize; j++) {
					g.setColor(colors[world.board[i][j]]);
					g.fillRect(topX + j * cellSize + 1,
							topY + i * cellSize + 1, cellSize - 1, cellSize - 1);

					g.setColor(Color.BLACK);
					char[] cha = new char[1];
					cha[0] = (char) ('0' + world.board[i][j]);
					g.drawChars(cha, 0, 1, topX + j * cellSize + 10, topY + i
							* cellSize + 20);
				}
			}
			...
			if (world.removeR != -1) {
				...
				for (int dr = 0; dr < 4; dr++) {
					for (int dc = 0; dc < 4; dc++) {
						g.drawLine(topX + world.removeC * cellSize + dc
								* cellSize / 2, topY + world.removeR * cellSize
								+ dr * cellSize / 2, topX + world.removeC
								* cellSize + (dc + 1) * cellSize / 2, topY
								+ world.removeR * cellSize + (dr + 1)
								* cellSize / 2);

						g.drawLine(topX + world.removeC * cellSize + (dc + 1)
								* cellSize / 2, topY + world.removeR * cellSize
								+ dr * cellSize / 2, topX + world.removeC
								* cellSize + dc * cellSize / 2, topY
								+ world.removeR * cellSize + (dr + 1)
								* cellSize / 2);
					}
				}
			...
			if (world.exchangeR1 != -1) {
				...
				g.drawLine(baseX2, baseY2, baseX2 - dr * 3 / 4 - dc, baseY2
						+ dc * 3 / 4 - dr);
				g.drawLine(baseX2, baseY2, baseX2 + dr * 3 / 4 - dc, baseY2
						- dc * 3 / 4 - dr);
				...
				g.drawLine(baseX1, baseY1, baseX1 - dr * 3 / 4 - dc, baseY1
						+ dc * 3 / 4 - dr);
				g.drawLine(baseX1, baseY1, baseX1 + dr * 3 / 4 - dc, baseY1
						- dc * 3 / 4 - dr);
			}
		...
		public void paint(Graphics g) {
			synchronized (world.worldLock) {
				...
				g.setFont(new Font("Arial", Font.BOLD, 16));
				g.drawString("Next " + PseudorandomGenerator.KEEP / 4
						+ " squares to appear:", 15, 50 + cellSize * boardSize);
				...
				for (int squareId = 0; squareId < PseudorandomGenerator.KEEP / 4; squareId++) {
					for (int r = 0; r < 2; r++) {
						for (int c = 0; c < 2; c++) {
							...
							int x = 15 + c * cellSize + squareId * 5 * cellSize
									/ 2;
	...
	public Drawer(World world, int cellSize) {
		...
		width = Math.max(boardSize * cellSize + EXTRA_WIDTH, NEXT_TILES_CNT
				* cellSize * 5 / 8 + EXTRA_WIDTH_NEXT);
...
public class SquareRemoverVis {
	...
	public int[] runSolution(TestCase tc, String execCmd) throws Exception {
		...
		try {
			try {
				...
				throw new Exception(
						"Unable to execute your solution using the provided command: "
								+ execCmd + ".");
			}
			...
			for (int i = 0; i < ans.length; i++) {
				try {
					...
					throw new Exception("Unable to read element " + i
							+ " (0-based) of your solution's return value.");
				}
			}
	...
	public int runTest() {
		...
		System.out.println("C = " + tc.colors);
	...
	public static void main(String[] args) throws Exception {
		...
		if (execCommand == null) {
			System.err
					.println("ERROR: You did not provide the command to execute your solution."
